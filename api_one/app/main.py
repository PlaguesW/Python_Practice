...
while True:
    ...
    try:
        conn = psycopg2.connect(host='localhost', database='postgres', user='postgres',
                                password='onlystr1ke', cursor_factory=RealDictCursor)
        cursor = conn.cursor()
...
my_posts = [{"title": "Best spot fo diving", "content": "Maybe Inndonezia", "id": 1},
...
async def delete_post(id: int, db: Session = Depends(get_db)):
    ...
    if post.first() == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"post with id: {id} doesn't extst")
